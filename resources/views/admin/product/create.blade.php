...
<div class="form-text">
    ...
    <input class="input-text  size" id="price" name="price" type="text" value="{{old('price')}}"  >
    @if($errors->has('price')) <span class="error">{{$errors->first('price')}}</span> @endif
</div>
<div class="form-text">
    <label class="label" for="quantity">{{trans('app.quantity')}}</label>
    <input class="input-text quantity" id="quantity" name="quantity" type="text" value="{{old('quantity')}}" >
    @if($errors->has('quantity')) <span class="error">{{$errors->first('quantity')}}</span> @endif
</div>
<div class="content">
    <label>{{trans('app.abstract')}}</label>
    <textarea row="50" cols="50" name="abstract">{{old('abstract')}}</textarea>
    @if($errors->has('abstract')) <span class="error">{{$errors->first('abstract')}}</span> @endif
</div>
<div class="content">
    <label>{{trans('app.content')}}</label>
    <textarea row="100" cols="50" name="content">{{old('content')}}</textarea>
...
<div class="input-radio">
    ...
    <input type="radio" name="status" value="closed" checked> closed<br>
</div>
...
<div class="form-submit">
    <input class="btn btn-opened" type="submit" value="{{trans('app.create')}}" >
</div>
</section>
</form>
@stop